...
#         # Idle threshold (calibrated based on normalized features)
#         self.IDLE_THRESHOLD = 4.0  # Calibrated for your system baseline
...
#         ransomware[:, 11] = np.random.uniform(0.3, 1.0, 30)
#         ransomware = np.abs(ransomware)
...
#         # Idle detection with calibrated threshold
#         is_idle = feature_sum < self.IDLE_THRESHOLD
        
#         # ML prediction only if not idle
#         if is_idle:
#             prediction_value = 1  # Normal
...
#             'honeypot_status': honeypot_status,
#             'recent_file_events': len(recent_files),
#             'suspicious_processes': sum(1 for p in recent_processes if p.get('threat_score', 0) > 30),
#             'is_idle': is_idle
#         }
...
"""
Stage 1 Integrated System - FIXED VERSION
...
# Import Stage 1 components - FIXED
print("Loading Stage 1 components...")

try:
    from file_monitor import FileMonitor
    print("✓ FileMonitor loaded")
...
try:
    from honeypot_manager import HoneypotManager
    print("✓ HoneypotManager loaded")
except Exception as e:
    print(f"✗ HoneypotManager error: {e}")
    exit(1)

try:
    from process_monitor import ProcessMonitor
    print("✓ ProcessMonitor loaded")
except Exception as e:
...
try:
    from ml_detector import RansomwareMLDetector
    print("✓ RansomwareMLDetector loaded")
except Exception as e:
    print(f"✗ RansomwareMLDetector error: {e}")
    exit(1)
